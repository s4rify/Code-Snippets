    ...
    for s = 1: length(flist)
        ...
        if isempty(find(strcmp(vals, 'Out'),1))
            ...
            if all(vals{1} == '???')
                ...
                warning('Missing values in the header file! Those are getting filled with NaNs.')
            else
            ...
            vals(all_ind_of_strings) = {1000};
            impedances(s,:) = cell2mat(vals);
            disp('Values out of range were replaces by the value 1000.')
        end
        % no impedance measurement was possible, BrainVisionRecorder fills in '?' instead of a value. let's
